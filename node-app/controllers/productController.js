...
module.exports.handledelete = (req, res) => {
    const { pid } = req.body; // Destructure the product ID from the request body
    
    Products.findByIdAndDelete(pid) // Find the product by ID and delete it
        .then((deletedProduct) => {
            if (deletedProduct) {
        ...
        .catch((err) => {
            console.error(err);
            res.status(500).send({ message: 'Server error. Failed to delete product.' });
        });
};
